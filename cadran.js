...
var HReelFige="temps réel";
var chrono; // gestion de setinterval
...
function HtoHmn(x){
  // converti un nombre en heure decimale en un texte H mn
  let y="";
  let x2=Math.abs(x);// passer en valeur absolue pour la conversion
  let signe="";
  let H=Math.floor(x2);
  let minutes=Math.round(((x2-H)*60));
  if (x<0){signe="-"}
  else {signe="+"}
  y=signe+ajouteZero(H)+" H "+ajouteZero(minutes)+" mn ";
  return y;
}
...
function heureAuto() {
  ...
  declinaisonSol();
  */
  if (HReelFige==="temps réel") { // affichage actuel de l'temps réel
    // figer l'heure
    clearInterval(chrono);
    HReelFige="temps figé";
    actualisation();
  }
  else {
    // defiger l'heure
    HReelFige="temps réel";
    actualisation();
    const idchrono=setInterval(actualisation, 1000);
    chrono=idchrono;
  }
}
...
function afficheHL(){
  ...
  if (HReelFige==="temps réel"){
    affiTemps=HReelFige+" ("+ajouteZero(HL.getSeconds())+")";
    document. getElementById("heureReelle").style.color = '#000000' ;
  }
  else {
    affiTemps=HReelFige;
    document. getElementById("heureReelle").style.color = '#F10000' ;
  }
  document.getElementById("idHL").textContent=formatHeure;
  document.getElementById("idGMT").textContent=decalageHL_texte+" H";
  document.getElementById("heureReelle").textContent=affiTemps;
}

// --- afiche coordoneees solaires
function afficheSol() {
  // pourquoi cette fonction est vide ?
}

// ---------------------------------------
//
// ----- calculs heure solaire moyenne ---
// ----- heure solaire vraie, heure UTC --
// -----        et affichages.      ------
//
// ---------------------------------------
...
function setHLmanuelle() {
  ...
  // arreter la mise a jour permanente de l'heure
  clearInterval(chrono);
  HReelFige="temps figé";
  actualisation();
...
let AHActuelle="-- H -- mn"; // valeur par defaut des affichages
let AhautActuelle="--°";
let AazActuelle="---°";
let AHLever="-- H -- mn"; // valeur par defaut des affichages
let AhautLever="0°";
let AazLever="---°";
let AHMidi="-- H -- mn"; // valeur par defaut des affichages
let AhautMidi="--°";
let AazMidi="180°";
let AHCoucher="-- H -- mn"; // valeur par defaut des affichages
let AhautCoucher="0°";
let AazCoucher="---°";

if (lat<=90){ // si la latitude est definie (inconnue correspond a lat=99) 
...
formatPosSol=" haut= "+Math.round(hautSolHeure/Math.PI*180)+"° , az="+ Math.round(azSolHeure/Math.PI*180+180)+" °"; // a supprimer ?
AhautActuelle=Math.round(hautSolHeure/Math.PI*180)+"°";
AazActuelle=Math.round(azSolHeure/Math.PI*180+180)+" °"
///......
...
AHLever=ajouteZero(Math.floor(hdLever))+" H "+ajouteZero(Math.round(frac(hdLever)*60))+" mn";
AazLever=Math.round(180-ecartAngulaire/Math.PI*180)+"°";

AHCoucher=ajouteZero(Math.floor(hdCoucher))+" H "+ajouteZero(Math.round(frac(hdCoucher)*60))+" mn";
AazCoucher=Math.round(180+ecartAngulaire/Math.PI*180)+"°";
}

else { // si la latitude n'est pas definie
  formatHautSol=", haut= --°"; // ?
}

// preparation des affichages

// on affiche l'heure actuelle dans tous les cas
AHActuelle=ajouteZero(HL.getHours())+" H "+ajouteZero(HL.getMinutes())+" mn";


document.getElementById("idHActuelle").textContent=AHActuelle;
document.getElementById("idhautActuelle").textContent=AhautActuelle;
document.getElementById("idazActuelle").textContent=AazActuelle;

document.getElementById("idHLever").textContent=AHLever;
document.getElementById("idhautLever").textContent=AhautLever;
document.getElementById("idazLever").textContent=AazLever;

document.getElementById("idHMidi").textContent=AHMidi;
document.getElementById("idhautMidi").textContent=AhautMidi;
document.getElementById("idazMidi").textContent=AazMidi;

document.getElementById("idHCoucher").textContent=AHCoucher;
document.getElementById("idhautCoucher").textContent=AhautCoucher;
document.getElementById("idazCoucher").textContent=AazCoucher;
...
function initialisation(){
  ...
  decalageHL=-1*HL.getTimezoneOffset()/60;
  majAffichages();  
  /*setInterval(actualisation,1000); /* Toutes les 1 seconde  */
  const idchrono=setInterval(actualisation, 1000);
  chrono=idchrono;
}
